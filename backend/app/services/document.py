...
from app.schemas.schemas import BulkReportPDFRequest
...
def generate_bulk_report_pdf(payload: BulkReportPDFRequest) -> str:
    """Generate a batch summary PDF report for bulk quoting."""
    ...
    output_dir = Path(settings.UPLOAD_DIR) / "quotes"
    output_dir.mkdir(parents=True, exist_ok=True)
    filename = f"BULK-QUOTE-{dt.utcnow().strftime('%Y%m%d%H%M%S')}.pdf"
    output_path = output_dir / filename
    ...
    report_title = payload.report_title or "Bulk Quote Summary"

    content = []
    content.append(Paragraph("CNC Quote Platform", title_style))
    content.append(Paragraph(report_title, heading_style))
    content.append(Paragraph(f"Generated: {dt.utcnow().strftime('%B %d, %Y %H:%M UTC')}", normal_style))
    content.append(Spacer(1, 10))

    customer_name = payload.customer_name or "Valued Customer"
    customer_company = payload.customer_company or ""
    ...
    summary_table = Table(
        [
            ["Total Files", str(payload.file_count)],
            ["Total Cost", f"{currency} {float(payload.total_cost):,.2f}"],
            ["Batch Lead Time", f"Up to {payload.max_lead_time_days:.1f} days"],
        ],
    ...
    content.append(Paragraph("Per-File Quote Details", heading_style))
    rows = [[
        "File",
        "Qty",
        "Material",
    ...
    for item in payload.items:
        rows.append([
            ...
            str(item.quantity),
            item.material_name,
            item.surface_finish_name,
            item.inspection_level_name,
            f"{float(item.unit_price):,.2f}",
            f"{float(item.line_total):,.2f}",
            f"{item.lead_time_days:.1f}d",
        ])

    details_table = Table(
        rows,
        colWidths=[3.2 * cm, 1.0 * cm, 2.1 * cm, 1.9 * cm, 2.0 * cm, 2.1 * cm, 2.2 * cm, 1.5 * cm],
        repeatRows=1,
    )
    ...
    if payload.notes:
        ...
        content.append(Paragraph(payload.notes, normal_style))

    content.append(Spacer(1, 14))
    content.append(Paragraph("Thank you for your inquiry. We look forward to working with you.", normal_style))
